...
from .training import DatasetBundle, TrainingConfig, evaluate, seed_everything, train_model
...
def config_hash(spec: dict) -> str:
    return hashlib.sha256(canonical_json({k: v for k, v in spec.items() if k != "seed"}).encode()).hexdigest()[:20]
...
def seed_streams(spec: dict) -> dict:
    ...
    return {
        "scheme": "sha256_named_streams_v1", "base_seed": spec["seed"],
...
def fit(spec: dict, p_layers: list[float], bundle: DatasetBundle, directory: Path,
        *, device: str, provenance: dict) -> dict:
    """Train one immutable output directory; never overwrite a prior run."""
    directory.mkdir(parents=True, exist_ok=False)
    randomization = seed_streams(spec)
    manifest = {"spec": spec, "p_layers": p_layers, "randomization": randomization,
                "split_hash": bundle.split_hash, "split_protocol": bundle.split_protocol,
                "test_subset_hash": bundle.test_subset_hash, "provenance": provenance}
    ...
    config = TrainingConfig(
        **{key: spec[key] for key in ("epochs", "batch_size", "learning_rate",
          "lr_floor_ratio", "weight_decay", "gradient_clip_norm", "evaluate_test")},
        seed=randomization["minibatch_seed"], stochastic_seed=randomization["dropout_seed"],
        restore_best_validation=True, device=device,
    )
    start = time.perf_counter()
    trained = train_model(model, optimizer, bundle, config,
                          return_best_state=True, return_final_state=True)
    history = {key: value.tolist() for key, value in trained.pop("history").items()}
    if any(not np.isfinite(history[key]).all() for key in
    ...
    test = {
        "best_validation_checkpoint": {"epoch": selected,
            "loss": trained["final_test_loss"], "accuracy": trained["final_test_accuracy"]},
        "fixed_final_epoch": {"epoch": spec["epochs"] - 1,
            "loss": trained["final_epoch_test_loss"], "accuracy": trained["final_epoch_test_accuracy"]},
    }
    for filename, state_key, endpoint in (
        ...
        torch.save({"model_state_dict": trained.pop(state_key), "spec": spec,
                    "p_layers": p_layers, "epoch": test[endpoint]["epoch"],
                    "split_hash": bundle.split_hash}, directory / filename)
    result = {"status": "complete", "selected_epoch": selected, "test": test,
              "history": history, "training": trained,
              "duration_seconds": time.perf_counter() - start,
              "checkpoint_sha256": {name: sha256_file(directory / name)
...
def synthetic_fixture():
    ...
    spec = {
        "stage": "confirm", "dataset": "fi2010", "model_kind": "mlp",
        "profile_id": "uniform", "mean_dropout": 0.1, "learning_rate": 0.01,
        "seed": 9182, "max_dropout": 0.20, "depth": 2, "width": 8, "heads": 8,
        "mlp_ratio": 4.0, "activation": "relu", "sigma_w_sq": 1.98,
        "sigma_b_sq": 0.0, "epochs": 4, "batch_size": 8, "weight_decay": 0.0,
        "lr_floor_ratio": 1e-3, "gradient_clip_norm": None, "train_size": 24,
        "validation_size": 12, "test_size": 12, "split_seed": 20260812,
        "evaluate_test": True, "budget_space": "dropout_probability",
        "parameterization": "sp", "cohort_id": "public-synthetic-endpoint-smoke-v1",
    }
    ...
    bundle = DatasetBundle(train, validation, test, digest, "fi2010",
                           "synthetic_v1", digest, "synthetic_v1", 0)
    return spec, bundle


def smoke(directory: Path) -> dict:
    spec, bundle = synthetic_fixture()
    result = fit(spec, [0.1, 0.1], bundle, directory, device="cpu",
                 provenance={**source_provenance(), "synthetic": True})
    if result["selected_epoch"] != 0 or not np.all(np.diff(result["history"]["validation_loss"]) > 0):
        raise AssertionError("Fixture must select epoch zero while validation loss rises")
    states = []
    for name, endpoint in (("best.pt", "best_validation_checkpoint"),
        ...
        np.testing.assert_allclose([loss, accuracy], [expected["loss"], expected["accuracy"]], rtol=1e-6)
    ...
    report = {"synthetic": True, "status": "complete", "selected_epoch": 0,
              "final_epoch": spec["epochs"] - 1, "saved_endpoints_reproduced": True}
...
def run(args):
    ...
    for name in args.profile:
        if name == "frontloaded":
            ...
            if name not in {"step_early", "big_step", "linear_early"}:
                raise ValueError("This cohort has no selected frontloaded arm; use an explicit profile ID")
    ...
    if args.seed is not None and (any(seed < 0 for seed in args.seed) or len(set(args.seed)) != len(args.seed)):
        raise ValueError("Seeds must be distinct nonnegative integers")
    if args.describe:
        print(json.dumps({"cohort": args.cohort, "split": cohort["split"],
                          "arms": dict(arms)}, indent=2))
        return
    ...
    print(json.dumps({"event": "verifying_data", "dataset": cohort["dataset"]}), flush=True)
    ...
    spec = arms[0][1]["spec"]
    split = cohort["split"]
    bundle = load_benchmark_bundle(spec["dataset"], root=args.data_root,
    ...
    if bundle.split_hash != split["split_hash"] or bundle.split_protocol != split["split_protocol"]:
        raise ValueError("Prepared data do not reproduce this cohort's frozen split; refusing to train")
    provenance = {**source_provenance(), "protocols_sha256": sha256_file(args.protocols),
                  "cache_sha256": cache_hash, "payload_sha256": payload_hash,
                  "standardization": split["standardization"], "cohort": args.cohort}
    for name, arm in arms:
        for seed in args.seed if args.seed is not None else arm["seeds"]:
            ...
            print(json.dumps({"event": "starting", "profile": name, "seed": seed}), flush=True)
            result = fit(spec, arm["p_layers"], bundle, destination, device=args.device,
                provenance={**provenance, "historical_seed": seed in arm["seeds"]})
            print(json.dumps({"event": "complete", "profile": name, "seed": seed,
                              "output": str(destination), "test": result["test"]}), flush=True)
...
def main():
    ...
    training.add_argument("--seed", nargs="+", type=int, help="default: all historical confirmation seeds")
    training.add_argument("--data-root", type=Path)
    training.add_argument("--output-dir", type=Path)
    training.add_argument("--cache-sha256", help="optional independently recorded cache file checksum")
    training.add_argument("--device", default="auto", choices=("auto", "cpu", "cuda"))
    training.add_argument("--describe", action="store_true", help="show frozen selection without loading data")
