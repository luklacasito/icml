...
def source_provenance():
    paths = [Path(__file__).resolve(), *sorted(SUPPORT.glob("*.py")),
             *sorted(SUPPORT.glob("*.json")), ROOT / "utils" / "training.py",
             ROOT / "utils" / "schedules.py"]
    hashes = {str(p.relative_to(ROOT)): sha256_file(p) for p in paths}
    return {"files": hashes, "sha256": hashlib.sha256(canonical(hashes).encode()).hexdigest()}
...
def specification(study, arm, seed, *, smoke=False):
    ...
    schedule = "constant" if arm == "uniform" else ("big_step" if study == "relu" else "reverse_step")
    get_schedule = relu.get_dropout_schedule if study == "relu" else vit_schedules.get_dropout_schedule
    recipe["dropout_probabilities"] = get_schedule(schedule, recipe["depth"], .1, .2)
    return {"schema_version": SCHEMA_VERSION, "study": study, "arm": arm, "seed": seed,
            "schedule": schedule, "recipe": recipe, "smoke": smoke,
            "protocol": "original_no_validation_fixed_recipe_extension_v1",
            "accuracy_unit": "percent", "selection": "none; test minima are retrospective",
...
def subset_indices(train_count, test_count, train_size, test_size, seed=0):
    ...
    train = rng.choice(train_count, train_size, replace=False) if train_size < train_count else np.arange(train_count)
    test = rng.choice(test_count, test_size, replace=False) if test_size < test_count else np.arange(test_count)
    return train, test
...
def prepare_inputs(images, targets, indices, device, study):
    ...
    mean = torch.tensor([.4914, .4822, .4465]).view(1, 3, 1, 1)
    std = torch.tensor([.2470, .2435, .2616]).view(1, 3, 1, 1)
...
def load_data(spec, data_root, device, expected_cache_sha256=None):
    ...
    if expected_cache_sha256 and cache_sha != expected_cache_sha256:
        raise RuntimeError(f"CIFAR cache hash mismatch: expected {expected_cache_sha256}, got {cache_sha}")
    recipe = spec["recipe"]
    tr, te = subset_indices(len(train_images), len(test_images), recipe["train_size"], recipe["test_size"], recipe["split_seed"])
    provenance = {"cache_files_sha256": cache, "cache_sha256": cache_sha,
                  "train_indices_sha256": array_hash(tr), "test_indices_sha256": array_hash(te),
                  "train_size": len(tr), "test_size": len(te), "split_seed": recipe["split_seed"],
    ...
    return (prepare_inputs(train_images, train_targets, tr, device, spec["study"]),
            prepare_inputs(test_images, test_targets, te, device, spec["study"]), provenance)


def build_model(spec):
    recipe = spec["recipe"]
    if spec["study"] == "relu":
        return relu.CriticalReLUNet(3072, recipe["width"], 10, recipe["dropout_probabilities"],
                                   sigma_w_sq=recipe["sigma_w_sq"], sigma_b_sq=recipe["sigma_b_sq"])
    return vit.ViT(recipe["depth"], recipe["embedding_dim"], recipe["heads"], recipe["mlp_ratio"],
                   recipe["dropout_probabilities"], ablation_mode="both")
...
def resolve_device(value):
    ...
    if device.type == "cuda":
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA was requested but is unavailable; refusing an unnoticed CPU run")
...
def verified_result(directory, spec, source, data):
    ...
    if result.get("status") != "complete" or any(len(result["curves"].get(key, [])) != epochs
            for key in ("train_loss", "test_loss", "train_accuracy_percent", "test_accuracy_percent")):
...
def run_trial(study, arm, seed, data_root, run_root, device, *, smoke=False, expected_cache_sha256=None):
    ...
    with fit_lock(directory):
        ...
        if existing is not None:
            print(canonical({"status": "skipped_verified_complete", "result": str(directory / "result.json")}), flush=True)
            return existing
        if device.type == "cuda":
        ...
        scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=recipe["epochs"], eta_min=lr_floor)
        criterion = torch.nn.CrossEntropyLoss()
        curves = {key: [] for key in ("train_loss", "test_loss", "train_accuracy_percent", "test_accuracy_percent", "learning_rate_after_step")}
        start = time.monotonic()
        for epoch in range(recipe["epochs"]):
            if study == "relu":
                train_loss, train_accuracy = relu.train_epoch(model, train_data, optimizer, criterion, recipe["batch_size"])
            else:
                train_loss, train_accuracy = vit_training.train_epoch(model, train_data, optimizer, criterion,
                    recipe["batch_size"], grad_clip=1., amp_dtype=amp_dtype)
            scheduler.step()
            if study == "relu":
                test_loss, test_accuracy = relu.evaluate(model, test_data, criterion, recipe["batch_size"])
            else:
                test_loss, test_accuracy = vit_training.evaluate(model, test_data, criterion, recipe["batch_size"], amp_dtype=amp_dtype)
            values = (train_loss, test_loss, train_accuracy, test_accuracy, optimizer.param_groups[0]["lr"])
            if not all(math.isfinite(value) for value in values):
                raise RuntimeError(f"Nonfinite metric at epoch {epoch}; not marking this fit complete")
            for key, value in zip(curves, values):
                curves[key].append(value)
            progress = {"status": "running", "spec": spec, "source": source, "data": data,
                        "runtime": runtime, "last_epoch_zero_based": epoch, "curves": curves,
            ...
            print(canonical({"study": study, "arm": arm, "seed": seed, "epoch": epoch + 1,
                             "epochs": recipe["epochs"], "train_loss": train_loss, "test_loss": test_loss,
                             "test_accuracy_percent": test_accuracy}), flush=True)
        ...
        atomic_write(checkpoint, lambda stream: torch.save({"model_state_dict": state, "spec": spec,
                     "source_sha256": source["sha256"], "data_sha256": data["sha256"],
                     "epoch_zero_based": final_epoch, "endpoint": "final_epoch"}, stream))
        result = {"status": "complete", "schema_version": SCHEMA_VERSION, "spec": spec,
                  ...
                  "metrics": {"final_epoch_test_loss": curves["test_loss"][-1],
                              ...
                              "test_accuracy_percent_at_retrospective_min_test_loss": curves["test_accuracy_percent"][minimum_epoch],
        ...
        print(canonical({"status": "complete", "result": str(directory / "result.json")}), flush=True)
        return result
...
def main():
    ...
    run.add_argument("--cache-sha256", help="Expected aggregate SHA256 of the official CIFAR batch/meta files")
    ...
    if args.command == "run":
        run_trial(args.study, args.arm, args.seed, args.data_root, args.run_root, args.device,
        ...
        for study in RECIPES:
            for arm in ("uniform", "frontloaded"):
                ...
                repeated = run_trial(study, arm, 900001, None, args.run_root, args.device, smoke=True)
                assert result == repeated
        print("Original model smoke and verified-completion restart checks passed.", flush=True)
