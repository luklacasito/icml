...
Additionally, we show that the two control parameters (dropout strength and distance to criticality) admit a scaling collapse into a single universal form,\footnote{This idea is directly borrowed from the scaling behavior of one-particle-irreducible vertex functions, or equivalently connected \(n\)-point functions, in critical quantum field theory: in the massless limit they obey homogeneous scaling forms after the appropriate field and distance rescalings \cite{ZinnJustin2002QFTCritical}.} and we illustrate how these predictions compare with mean-field recursions. 
...
\[
\frac{\partial f_{\rm sing}}{\partial m}
=
-h-tm+\frac{g_\rho}{2}m^2,
\]
...
The remaining tie-breaker comes from representation dynamics beyond the mean-field closure. Rank collapse gives one concrete mechanism \cite{dong2021attention,noci2022signalpropagation}: token diversity in pure attention, and analogous rank-diversity proxies in wide MLPs, tend to decay with depth. Dropout pushes in the opposite direction at the level of second moments by inflating diagonal variance and shrinking normalized correlations, thereby increasing a participation-ratio effective rank (App.~\ref{app:rank_flow}). This mechanism is preventative rather than reparative: once the representation has collapsed to an essentially one-dimensional signal subspace, inverted dropout mostly adds training-time diagonal noise rather than constructing new task-relevant modes. Thus rank flow breaks the permutation symmetry of \(\xi_{\rm eff}\) by favoring concentrated dropout where collapse is fastest; in regularization- and rank-collapse-limited regimes, this means front-loading the dropout budget.
...
\bottomrule
\end{tabular}
\caption{Final-epoch improvements over the corresponding constant-dropout baseline. Accuracy gains are shown both as percentage-point differences and as relative percent improvements over the baseline final accuracy. Loss reductions are relative reductions in cross-entropy.}
...
The mean-field propagation objective is invariant under permutations of the depth profile \(h_\ell\), so it fixes the amount of dropout but not its ordering. Rank collapse supplies one beyond-mean-field tie-breaker. In pure self-attention, for token representations \(X_\ell\in\mathbb R^{n\times d}\), and ignoring residual connections, \(X_{\ell+1}=A_\ell X_\ell\). Hence
\begin{equation}
\operatorname{rank}(X_{\ell+1})
\;\leq\;
\min\{\operatorname{rank}(A_\ell),\operatorname{rank}(X_\ell)\}.
\end{equation}
...
R(\ell)-1
\;\lesssim\;
\bigl(R(0)-1\bigr)\exp\!\left(-c\,2^\ell\right),
\end{equation}
for an architecture-dependent constant \(c>0\). In wide MLPs, analogous rank-diversity proxies typically decay exponentially with depth, e.g.
\begin{equation}
R(\ell)-1
\;\sim\;
...
For the literal bias-free ReLU map, \(\bar F_\rho(c)=\rho F_{\mathrm{ReLU}}(c)\), so \(\chi_\rho=\rho\) and \(t=\rho-1=-h\). Thus the \(t=0,h>0\) direction below is the kinked normal-form field direction, while standard ReLU dropout follows a constrained path through the same scaling function.
...
\end{equation}
which reproduces the characteristic \(1-c_\ell\sim \ell^{-2}\) relaxation of ReLU activations at the edge-of-chaos and gives \(\theta_{\rm rel}=2\).
...
This appendix collects the experimental details and additional figures supporting the scheduling predictions. Code for the experiments and figure generation is available at \url{https://github.com/luklacasito/dropout-mft-experiments/tree/55da5ad6bec6f2d93b40d65b56ec4e1b33bc53ec}.
