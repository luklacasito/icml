...
\begin{abstract}
We develop a mean-field theory of dropout as a perturbation of critical signal propagation at the edge of chaos. Dropout shifts the perfect-alignment fixed point, making the depth scale for information propagation finite even at critical initialization. We derive critical and crossover scaling laws for correlation decay and establish that smooth activations and kinked (ReLU-type) activations constitute distinct universality classes, with different critical exponents and a universal two-parameter scaling collapse in detuning and dropout strength. The distinction traces to the analytic structure of the correlation map: smooth activations admit a Taylor expansion near perfect alignment, while kinked activations develop a branch point with universal non-analyticity. As a corollary, the framework yields principled dropout schedules that maximize effective correlation length under fixed budget, giving higher validation accuracy and lower validation loss at no extra computational cost. We test the predictions in MLPs and Vision Transformers and discuss CNN/ResNet extensions.
\end{abstract}

\section{Introduction}

Mean-field analyses of randomly initialized deep networks reveal an order-to-chaos phase diagram that controls both signal propagation and gradient penetration with depth \cite{poole2016exponentialexpressivitydeepneural,schoenholz2017deepinformationpropagation,Bahri2020StatMechDeepLearning,bahri2024leshoucheslecturesdeep,Halverson:2024hax}. In this work, we give a holistic exploration of how this picture is modified by dropout. Dropout is a relevant perturbation: it displaces the critical fixed point and grows under representation-group (coarse-graining) flow \cite{roberts2022principlesdeeplearningtheory,bahri2024leshoucheslecturesdeep}, pushing the dynamics away from criticality and determining the macroscopic phase. Concretely, dropout deforms the correlation map \cite{schoenholz2017deepinformationpropagation} by adding a correlation-independent shift at perfect alignment, so that perfect correlation between inputs is no longer a fixed point for any nonzero dropout. Thus, even at the edge-of-chaos \cite{sompolinsky1988chaos,packard1988adaptation,bertschinger2004real}, the depth correlation length is finite. We interpret this shift as an equation of state for the decorrelation order parameter $m \equiv 1-c^\ast$ (with $c^\ast$ the asymptotic inter-signal correlation), derive the resulting scaling laws, and show that smooth and kinked (ReLU-type) activations exhibit the same qualitative phase diagram but different critical scaling in the presence of dropout.

Additionally, we show that the two control parameters (dropout strength and distance to criticality) admit a scaling collapse into a single universal form,\footnote{This idea is directly borrowed from the scaling behavior of one particle irreducible vertex functions in quantum field theory, which exhibit a similar scaling collapse in the massless limit.} and we illustrate how these predictions compare with mean-field recursions. 

This analysis yields three main conceptual contributions. First, rather than destroying the mean-field fixed-point structure as previously believed \cite{schoenholz2017deepinformationpropagation}, dropout displaces the perfect-alignment point to a nontrivial $c^\ast<1$ fixed point. This keeps the correlation length recursion and yields a Landau equation of state for $m=1-c^\ast$ together with a two-parameter scaling collapse. Second, we show that universality classes and scaling laws are set by activation smoothness: smooth and kinked activations have distinct critical exponents (Table~\ref{tab:critical_exponents}), a distinction reflected in their Hermite spectral structure (App.~\ref{sec:var-opt-act}). Since the same Gaussian activation kernels arise beyond MLPs, including CNNs and residual branches (App.~\ref{app:cnn_extension}), we expect the qualitative smooth/kinked split to be a well-motivated heuristic. Third, we treat dropout as a depth-dependent dynamical field, and optimizing the effective correlation length functional with respect to this field yields first-principles dropout schedules that we validate experimentally at fixed budget. Treating classically fixed hyperparameters as fields that can change through the network opens a new dimension for hyperparameter optimization across depth, and potentially over training time.


This work complements prior studies of depth-dependent regularization: stochastic depth drops whole residual blocks with increasing probability \cite{huang2016deep}, curriculum dropout anneals dropout over training time \cite{morerio2017curriculum}, and LayerDrop removes transformer layers for efficiency \cite{fan2019reducing}. Our schedules are spatial depth profiles, so they are complementary to temporal curricula and layer-dropping mechanisms. A distinction in the topology of the critical surface for kinked and smooth activations was previously observed in \cite{hayou2019impact}; here we extract the scaling exponents and identify the corresponding universality classes.



\section{Mean-field theory background}
\label{sec:mft_background}
We first state the mean-field assumptions for MLPs, which provide the baseline for the dropout analysis \cite{poole2016exponentialexpressivitydeepneural,schoenholz2017deepinformationpropagation,Bahri2020StatMechDeepLearning,bahri2024leshoucheslecturesdeep,Halverson:2024hax}. We consider fully-connected MLPs at random initialization, where preactivations are well-approximated as Gaussian random variables with self-consistently determined covariances. In the strict $N_l\to\infty$ limit this Gaussian description becomes exact \cite{Lee2018DNNasGP}, while at large but finite width deviations from Gaussianity can be organized perturbatively in the depth-to-width aspect ratio (schematically $L/N$), as treated comprehensively in \cite{roberts2022principlesdeeplearningtheory,bahri2024leshoucheslecturesdeep}.
...
\begin{equation}
W^{l}_{ij}\sim \mathcal{N}\!\left(0,\frac{\sigma_{w}^{2}}{N_{l}}\right),
\qquad
b^{l}_{i}\sim \mathcal{N}\!\left(0,\sigma_{b}^{2}\right),
\end{equation}
take the input as $y^{0}_{i;a}=x_{i;a}$, and propagate forward via
\begin{equation}
z^{\,l}_{i;a}= W^{l}_{ij}\,y^{\,l}_{j;a}+b^{l}_{i},
\qquad
y^{\,l+1}_{i;a}=\phi\!\left(z^{\,l}_{i;a}\right),
\end{equation}
...
u_{1}&=\sqrt{q^{\,l-1}_{aa}}\,z_{1},\qquad 
c^{\,l-1}_{ab}=\frac{q^{\,l-1}_{ab}}{\sqrt{q^{\,l-1}_{aa}\,q^{\,l-1}_{bb}}},\\
...
\begin{equation}
|{c^*-c^l_{ab}}|\propto e^{-l/\xi_{c}},
\qquad
\xi_{c}^{-1}\equiv -\log|r_{c}|.
\label{corrq}
\end{equation}
...
As previously discussed, the mean field theory is an expansion in Non-Gaussianities suppressed by powers of \(L/N \ll 1\), therefore, in our experiments we work in a controlled regime with \(L/N \ll 1\) (i.e., \(N \gg L\)), which is also the regime in which networks perform best empirically \cite{roberts2022principlesdeeplearningtheory}. The correlation length diverges as we approach criticality, amplifying finite-width and other non-Gaussian corrections. Thus, when probing critical power laws with dropout, we keep the dropout rate small but non-negligible: infinitesimal dropout would require prohibitively deep networks to estimate the correlation length and related quantities.

Although we derive these results for MLPs, analogous large-width limits exist elsewhere. CNNs become near-Gaussian at infinite channel count \cite{cnnslecun,xiao2018dynamicalisometrymeanfield}. ResNets are especially suggestive: Yang and Schoenholz \cite{yang2017meanfieldresnets} find qualitatively different tanh/ReLU large-depth behavior. Skip connections alter global depth dynamics, changing exponential convergence to subexponential or polynomial laws and allowing norm drift, but each residual branch still inherits the local nonlinear Gaussian kernel. Thus our smooth/kinked universality split offers a possible theoretical explanation for their observed dichotomy; a dropout-deformed ResNet theory is left for future work. For transformers, related infinite-width analyses use large hidden dimension and/or many heads \cite{vaswani2023attentionneed,Hron2020InfiniteAttention,Halverson:2024hax}. Because the exponents are controlled by the local analytic structure of this channel, we expect the mechanism to persist when it controls the wide-limit recursion. In Sec.~\ref{subsec:optimal_dropout}, we test this extrapolation in transformers.

In App.~\ref{app:experimental_info} we probe realistic scenarios across architectures, datasets, and schedules. We focus on overparameterized regimes where dropout improves generalization and accuracy, and we explicitly study the trade-off between dropout-driven regularization and stable signal propagation. The mean-field objective predicts sparse, step-like allocation at fixed budget, but it is permutation invariant in depth and therefore cannot by itself decide where dropout should be concentrated. To break this degeneracy we appeal to effects outside the mean-field closure, such as early coadaptation and rank collapse: these make the first layers the most valuable place to inject noise, explaining why front-loaded schedules work best in the experiments.
...
\[
p^{(\ell)}_{j} \sim \mathrm{Bernoulli}(\rho),
\qquad
y^{(\ell)}_j \;\mapsto\; \tilde y^{(\ell)}_j \equiv \frac{p^{(\ell)}_{j}}{\rho}\,y^{(\ell)}_j,
\]
i.e.\ the standard inverted-dropout convention in which $\mathbb E_p[\tilde y^{(\ell)}_j]=y^{(\ell)}_j$, and hence $\mathbb E_p[\tilde z^\ell_i\mid W,b]=z^\ell_i$, so the mean preactivation is invariant to the dropout rate \cite{schoenholz2017deepinformationpropagation}. Throughout, $\rho$ is the keep probability. When comparing two inputs, we draw the dropout masks independently for each input\footnote{Correlated dropout masks can alleviate this detuning: independent dropout masks across datapoints inject sample-specific multiplicative noise \cite{bishop1995noise} that destroys the \(c=1\) fixed point. Instead, if the same mask is shared across the batch, the fixed point structure is preserved and near critical scaling can be recovered by retuning, but the noise becomes batch coupled and the regularization is correspondingly weaker and more structured. We leave detailed analysis of correlated masks to future work.}; this independence is the source of the decorrelation effect analyzed below.

We establish that activation smoothness partitions networks into distinct universality classes, which govern their asymptotic scaling near the edge-of-chaos and determine how dropout cuts off the depth correlation length. This distinction was motivated by a variational problem for the correlation length: in the near-critical Gaussian channel, Hermite polynomials form a natural basis, and common activations such as $\tanh$ or ReLU admit a Hermite decomposition whose coefficients control how strongly the activation perturbs long-range correlation propagation. The smoking gun is that kinks produce algebraic (power-law) tails in the Hermite spectrum, spreading weight across many degrees and thus many effective scales, whereas smooth (more precisely, analytic) activations have rapidly decaying coefficients, with spectral weight concentrated in low-order modes and an essentially exponential falloff. The details of these computations can be found in App.~\ref{sec:var-opt-act}.

Our classification is orthogonal to the scale-invariant vs. non-scale-invariant distinction of \cite{roberts2022principlesdeeplearningtheory}: the multi-scale Hermite spectrum highlighted above is controlled primarily by analytic regularity (kinks and other non-analytic structure), not by scale invariance per se. The criterion is based on two facts: (a) near-critical scaling laws differ between smooth and kinked activations, and (b) the Hermite spectral weight of non-analytic activations decays as a power law, in contrast to the exponential decay characteristic of smooth activations.
...
To begin, we drop activations at random with keep probability $\rho$, so the preactivations are
\begin{equation}
z_i^\ell
=
\frac{1}{\rho} \sum_j W_{ij}^\ell \,p_j^\ell \,y_j^\ell
+
b_i^\ell.
\end{equation}
...
\bar{q}_{aa}^{\,\ell}
=
\frac{\sigma_{w}^{2}}{\rho} \int \! Dz\, \phi^{2}\!\left( \sqrt{\bar{q}_{aa}^{\,\ell-1}}\, z \right)
+
\sigma_{b}^{2},
\end{equation}
...
\begin{equation}
  \bar q^{*}
  \;=\;
  \frac{\sigma_w^2}{\rho}\int Dz\,\phi^2\!\big(\sqrt{\bar q^{*}}\,z\big)
  +
  \sigma_b^2.
...
1
-
\frac{1 - \rho}{\rho \bar{q}^{*}} \sigma_{w}^{2} \int \! Dz\, \phi^{2}\!\left( \sqrt{\bar{q}^{*}}\, z \right).
\end{equation}
...
\begin{align}
  ...
  \\
  &\;=\;
  \frac{1-\rho}{\rho \bar q^{*}}\;
  \sigma_w^2\!\int Dz\,\phi^2\!\big(\sqrt{\bar q^{*}}\,z\big).
...
\Rightarrow
h>0,$
so the fixed point at $c^*=1$ is immediately spoiled: even if two inputs are perfectly aligned at some depth, the next layer produces $\bar c_{ab}=1-h<1$. Since we focus on weak dropout, it is useful to record the small-\(\delta\rho\) expansion. Writing $\delta\rho\equiv 1-\rho$ with $\delta\rho\ll 1$, we have
$\bar q^*=q^*+\mathcal O(\delta\rho)$, so to leading order we have
\begin{equation}
  h
  =
  \delta\rho\;
  \frac{\sigma_w^2}{\bar q^{*}}\!\int Dz\,\phi^2\!\big(\sqrt{\bar q^{*}}\,z\big)
  +
  \mathcal O(\delta\rho^2).
  \label{eq:Delta_weakdrop}
\end{equation}

The same dropout asymmetry appears in the backward pass. Let \(\delta^\ell_{j,a}\) denote the backpropagated preactivation gradient for input \(a\). With inverted dropout the local backward recursion contains the same mask derivative,
\[
\delta^\ell_{j,a}
=
\frac{p^\ell_{j,a}}{\rho}\,
\phi'(u^\ell_{j,a})
\sum_i W^{\ell+1}_{ij}\delta^{\ell+1}_{i,a},
\]
up to the harmless convention-dependent shift in layer indices. If \(G^\ell_{ab}\equiv \mathbb E[\delta^\ell_a\delta^\ell_b]\) denotes the two-input gradient covariance in the wide-limit recursion, then
\[
G^\ell_{ab}
=
\sigma_w^2
\mathbb E\!\left[
\frac{p^\ell_a p^\ell_b}{\rho^2}
\phi'(u_a)\phi'(u_b)
\right]
G^{\ell+1}_{ab}.
\]
Without dropout this is propagated by the same local susceptibility that controls forward correlations \cite{schoenholz2017deepinformationpropagation}. With independent dropout masks, however, the diagonal channel has \(\mathbb E[(p_a)^2]/\rho^2=1/\rho\), while for \(a\neq b\), \(\mathbb E[p_a p_b]/\rho^2=1\). Thus dropout inflates the single-sample backward variance while leaving the cross-sample channel uninflated, exactly mirroring the forward variance/covariance asymmetry that produced the field \(h\). After normalization, perfect alignment of backward signals is displaced away from \(1\) by a dropout-controlled field \(h_\nabla(\rho)\). The microscopic map from \(\rho\) to \(h_\nabla\) differs from the forward map, but the local equation-of-state structure is the same, so the dropout cutoff of the backward trainable depth has the same critical exponents as the forward correlation length.
...
For a smooth normalizable activation \(\phi\in C^2\), repeated application of Price’s theorem gives the first two
...
\begin{align}
  ...
  \bar F_\rho'(1)
  =
  \sigma_w^2\!\int Dz\,\big[\phi'(\sqrt{\bar q^{*}}\,z)\big]^2,
  ...
  \bar F_\rho''(1)
  =
  \sigma_w^2\,\bar q^{*}\!\int Dz\,\big[\phi''(\sqrt{\bar q^{*}}\,z)\big]^2.
  \label{eq:g_dropout}
\end{align}
For kinked activations (ReLU-type) \(\bar F_\rho''(1)\) is not finite and the analytic expansion below is not the
...
\begin{equation}
  h + t\,m - \frac{g_\rho}{2}\,m^2 = 0,
  \quad\Longleftrightarrow\quad
  h = \frac{g_\rho}{2}\,m^2 - t\,m,
...
\begin{equation}
  m(t,h)
  \;=\;
  \frac{t + \sqrt{t^2 + 2 g_\rho h}}{g_\rho}.
...
\toprule
\textbf{Universality Class} & $\boldsymbol{\nu_t}$ & $\boldsymbol{\beta}$ & $\boldsymbol{p}$ & $\boldsymbol{\gamma}$ & $\boldsymbol{\delta}$ & $\boldsymbol{\nu_\rho}$ & $\boldsymbol{\alpha}$ \\
\midrule
...
\bottomrule
\end{tabular}
\caption{Critical exponents for neural network initialization across different universality classes. }
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=1.0 \textwidth]{figures/theory/critical_exponents.png}
\caption{
...
Dropout and subcritical detuning exhibit an exact theoretical correspondence, allowing the equation of state \eqref{eq:EoS_correct} to admit a single-variable scaling collapse. This is expected from the point of view of statistical mechanics where universal scaling collapse is a hallmark of criticality. 
In practice, experiments are rarely tuned exactly to the correlation edge \(t=0\) while simultaneously taking the dropout field to zero, \(h\to 0\). The correct near-critical description is therefore a two-parameter scaling function.

For smooth activations, the order parameter \(m\equiv 1-c^*\) obeys \eqref{eq:EoS_correct},
\begin{equation}
h
=
\frac{g_\rho}{2}\,m^2 - t\,m,
\qquad
t \equiv \chi_\rho - 1.
\label{eq:eos_smooth_correct}
\end{equation}
This implies the explicit crossover solution
\begin{equation}
m(t,h)
=
\frac{t+\sqrt{t^2+2g_\rho h}}{g_\rho}.
\label{eq:m_smooth_crossover_correct}
\end{equation}
...
\end{equation}
showing that kinked activations lie in a distinct universality class from smooth activations.

\begin{figure*}[ht]
    \centering
    \includegraphics[width=1.0\textwidth]{figures/theory/smooth_scaling_collapse.png}
    \caption{
        Two-parameter crossover and scaling collapse of the dropout-deformed equation of state for the smooth universality class (tanh). Plots obtained using MFT recursion relations. The figure displays the relationship between different variables collapsing into a universal function as $\bar \tau$ increases. The kinked counterpart is shown in Fig.~\ref{fig:kinked_collapse}.}
    \label{fig:uni_scaling}
\end{figure*}


Figure~\ref{fig:uni_scaling} shows this collapse for smooth activations. Practically, smooth activations preserve longer correlation lengths at weak dropout, while kinked activations have a wider crossover window and are more forgiving to imperfect tuning; neither class is uniformly preferable.
...
\begin{equation}
\delta c_L \;\sim\; \prod_{\ell=1}^L \lambda_\ell \,\delta c_0,
\qquad
\lambda_\ell \;\equiv\; \lambda(t,h_\ell),
...
\xi_{\rm eff}^{-1}
\;\equiv\;
-\frac{1}{L}\sum_{\ell=1}^L \log\lambda_\ell
\;\simeq\;
\frac{1}{L}\sum_{\ell=1}^L \sqrt{t^2+2g_\rho h_\ell}.
\end{equation}
At the correlation edge $t=0$, this reduces to
\begin{equation}
\xi_{\rm eff}^{-1}(t=0)
\;\simeq\;
\sqrt{2g_\rho}\,\frac{1}{L}\sum_{\ell=1}^L h_\ell^{1/2}
\;\propto\;
\frac{1}{L}\sum_{\ell=1}^L h_\ell^{1/2}.
\label{eff_corr}
\end{equation}
...
\begin{equation}
\min_{h}\int_0^1 h(x)^{1/2}\,dx
\qquad
{\rm s.t.}\qquad
\int_0^1 h(x)\,dx=\bar h.
\end{equation}
Since $h^{1/2}$ is concave, Jensen's inequality implies that any constant schedule maximizes $\int h^{1/2}$ at fixed mean; hence any minimizer must saturate the box constraint and is step-like. Equivalently, any schedule that takes values in $\{0,h_{\max}\}$ with active fraction $f=\bar h/h_{\max}$ is optimal. A convenient representative is a step schedule,
\begin{equation}
h(x)=
\begin{cases}
0,& x<1-f,\\
...
f=\frac{\bar h}{h_{\max}},
\end{equation}
for which uniform dropout yields $\xi_{\rm eff}\sim \bar h^{-1/2}$ while the step schedule gives
\begin{equation}
\frac{\xi_{\rm eff,step}}{\xi_{\rm eff,uniform}}
=
\sqrt{\frac{h_{\max}}{\bar h}}
\;\ge\;
1.
\end{equation}
For kinked activations at $t=0$, one has $\xi^{-1}\propto h^{1/3}$, so the objective becomes $\int_0^1 h(x)^{1/3}dx$, which is also concave and therefore leads to the same step-like allocation.
...
Choose a mean field budget $\bar h$ and a local cap $h_{\max}$, set the active fraction $f=\bar h/h_{\max}$, and allocate dropout at the cap on approximately an $f$ fraction of layers. Correlation dynamics alone leaves the depth ordering unresolved; in regimes where early coadaptation or rank collapse is expected, front-load it. In residual or transformer-like models, a linearly decreasing schedule is a smoother version of the same bias and can be preferable empirically.

A tie-breaker arises from representation dynamics beyond the mean-field closure. One concrete mechanism is rank collapse \cite{dong2021attention,noci2022signalpropagation}. In pure self-attention, for token representations $X^\ell\in\mathbb{R}^{n\times d}$ one has (in the absence of residual connections)
\begin{equation}
\mathrm{rank}\!\left(X^{\ell+1}\right)
\;\leq\;
\mathrm{rank}\!\left(A^\ell\right)\,\mathrm{rank}\!\left(X^\ell\right),
\end{equation}
where $A^\ell$ is the attention matrix. Since $A^\ell$ contracts geometrically toward the rank-one projector $\mathbf{1}\mathbf{1}^\top/n$, an effective-rank proxy collapses doubly exponentially with depth,
\begin{equation}
R(\ell)
\;\lesssim\;
R(0)\,\exp\!\left(-c\,2^\ell\right),
\end{equation}
for an architecture-dependent constant $c>0$. In wide MLPs, rank proxies typically decay exponentially with depth (e.g. at a rate set by a Lyapunov gap),
\begin{equation}
R(\ell)-1
\;\sim\;
\bigl(R(0)-1\bigr)\exp\!\left(-2(\lambda_1-\lambda_2)\,\ell\right),
\end{equation}
...
\tilde y_{\ell,i}
\;=\;
\frac{p_{\ell,i}}{\rho_\ell}\,y_{\ell,i},
\qquad
p_{\ell,i}\sim \mathrm{Bernoulli}(\rho_\ell),
\end{equation}
with the $p_{\ell,i}$ independent across $i$. Let the (uncentered) second moment at depth $\ell$ be
\begin{equation}
\Sigma_\ell
\;\equiv\;
\mathbb{E}\!\left[y_\ell y_\ell^\top\right],
\end{equation}
and define $\tilde\Sigma_\ell$ analogously for $\tilde y_\ell$. Averaging over the dropout mask gives the exact second-moment update
\begin{equation}
\tilde\Sigma_\ell
\;=\;
\Sigma_\ell
+
\left(\frac{1}{\rho_\ell}-1\right)\mathrm{diag}(\Sigma_\ell),
...
D_\ell
\;\equiv\;
\mathrm{diag}(\Sigma_\ell),
\qquad
C_\ell
\;\equiv\;
D_\ell^{-1/2}\Sigma_\ell D_\ell^{-1/2},
\end{equation}
one obtains the normalized-correlation shrinkage identity
\begin{equation}
\tilde C_\ell
\;=\;
\rho_\ell\,C_\ell + (1-\rho_\ell)\,I,
\label{eq:dropout_corr_shrinkage}
\end{equation}
which explicitly suppresses off-diagonal correlations and pushes the spectrum toward isotropy. A convenient quantitative proxy is the participation-ratio effective rank
\begin{equation}
R_{\mathrm{eff}}(\Sigma)
\;\equiv\;
\frac{\bigl(\mathrm{tr}\,\Sigma\bigr)^2}{\mathrm{tr}\!\left(\Sigma^2\right)}.
\end{equation}
Combining \eqref{eq:dropout_second_moment} with trace identities yields the closed form
\begin{equation}
R_{\mathrm{eff}}\!\left(\tilde\Sigma_\ell\right)
\;=\;
\frac{\bigl(\mathrm{tr}\,\Sigma_\ell\bigr)^2}{
\rho_\ell^2\,\mathrm{tr}\!\left(\Sigma_\ell^2\right)
+
\left(1-\rho_\ell^2\right)\mathrm{tr}\!\left(\mathrm{diag}(\Sigma_\ell)^2\right)},
\end{equation}
and since $\mathrm{tr}(\Sigma_\ell^2)\ge \mathrm{tr}(\mathrm{diag}(\Sigma_\ell)^2)$ for any PSD $\Sigma_\ell$, it follows that
\begin{equation}
R_{\mathrm{eff}}\!\left(\tilde\Sigma_\ell\right)\;\ge\;R_{\mathrm{eff}}(\Sigma_\ell),
\end{equation}
with equality only when $\Sigma_\ell$ is diagonal. Thus dropout increases effective rank at the level of second moments by injecting entropy and suppressing correlation buildup.
Independent additive noise would also inflate the diagonal and break perfect alignment; dropout is the multiplicative Bernoulli realization for which the effective field is directly controlled by $\rho_\ell$. Thus the Landau form can describe a broader class of sample-wise noise sources, with different microscopic maps to $h$.

This mechanism is inherently preventative: if the representation has already collapsed to an essentially one-dimensional signal subspace $\Sigma_\ell\approx vv^\top$, then \eqref{eq:dropout_second_moment} primarily adds diagonal variance rather than constructing new task-relevant modes, and under inverted dropout this injected variance is absent at inference. Consequently, at fixed dropout budget the mean-field propagation objective prefers step-like schedules but does not care about ordering, while rank flow breaks the permutation symmetry by favoring concentrated dropout where collapse is fastest.  In regularization and rank-collapse limited regimes (especially in attention-dominated models), front-loading dropout acts as an entropic buffer that preserves higher-rank representations through the early layers, after which later layers can compress and refine a richer set of task-relevant features.

To test these predictions, we deliberately work in controlled regimes where mean-field assumptions remain valid (width $\gg$ depth, moderate dropout) rather than pursuing benchmark performance. We evaluate MLPs and Vision Transformers \cite{dosovitskiy2021vit} using CIFAR-10/100 \cite{krizhevsky2009cifar} (full details in App.~\ref{app:experimental_info}), where finite-width corrections and extensive data augmentation do not obscure the signal-propagation effects central to our analysis. Across architectures, early step-like schedules consistently outperform constant dropout (e.g., ViT on CIFAR-100: linearly decreasing achieves $49.38\%$ vs $48.69\%$ constant, $p < 0.05$), confirming that the scheduling principle transfers beyond the MLP setting in which it was derived. Appendix~\ref{app:sweeps} adds sweeps over $\bar h$, width, and activation class, including kinked ReLU and smooth GELU MLPs: the advantage persists over the perturbative dropout range and weakens only at high dropout or in overly narrow networks, precisely where the theory exits its regime of validity.
...
By extending mean-field signal propagation to include multiplicative dropout, we established that activation non-analyticity fundamentally dictates universality. This formalism yields a Landau equation of state for the decorrelation order parameter, a universal two-parameter scaling collapse, and distinct critical exponents for smooth versus kinked channels. As a demonstration that the framework has practical predictive power, we derived dropout schedules from the theory and tested them experimentally; the predicted front-loaded schedules consistently outperform constant dropout. Extending this mean-field analysis to attention mechanisms and incorporating finite-width corrections are natural directions for future work.
...
The main practical benefit is lower training cost at fixed performance; we do not anticipate additional ethical risks.

\textbf{Conflict of Interest Disclosure.} The author has no financial conflicts of interest to disclose.
...
Here we briefly review the mean-field formalism of deep information propagation (DIP) \cite{schoenholz2017deepinformationpropagation}, with broader background in the Les Houches and TASI lecture notes \cite{bahri2024leshoucheslecturesdeep,Halverson:2024hax}, which will serve as the baseline for what follows. The context is a fully-connected MLP at random initialization, studied in the wide limit, where preactivations are well-approximated as Gaussian random variables with self-consistently determined covariances. In the strict $N_l\to\infty$ limit this Gaussian description closes, while at large but finite width deviations from Gaussianity can be organized perturbatively in the depth-to-width aspect ratio (schematically $L/N$), as emphasized in \cite{roberts2022principlesdeeplearningtheory}.

We consider a depth-$L$ network with $N_l$ Neurons in layer $l$, and an activation $\phi:\mathbb{R}\to\mathbb{R}$ (e.g.\ $\tanh$, ReLU, \emph{etc.}). We use the standard i.i.d.\ Gaussian initialization
\begin{equation}
W^{l}_{ij}\sim \mathcal{N}\!\left(0,\frac{\sigma_{w}^{2}}{N_{l}}\right),
\qquad
b^{l}_{i}\sim \mathcal{N}\!\left(0,\sigma_{b}^{2}\right),
...
The mean-field approximation is that, at large width, each preactivation is a sum of many weakly-dependent terms and is therefore well modelled by a Gaussian with matching first two moments. In the infinite-width limit this becomes exact layer-by-layer, while at finite width the first layer is exactly Gaussian (being a linear map of fixed inputs) and subsequent layers are only approximately Gaussian, with controlled non-Gaussian corrections.

We first track the layer-wise preactivation variance for a single input $a$,
\begin{equation}
q^{\,l}_{aa}:=\mathbb{E}\!\left[\left(z^{\,l}_{i;a}\right)^{2}\right],
\qquad
\mathbb{E}\!\left[z^{\,l}_{i;a}\right]=0 .
\end{equation}
At initialization, $q^{\,l}_{aa}$ is deterministic in MFT and evolves by a closed recursion. Conditioning on the previous layer and using independence and zero means gives
\begin{equation}
q^{\,l}_{aa}
=
\sigma_{w}^{2}\int Dz\,\phi^{2}\!\left(\sqrt{q^{\,l-1}_{aa}}\,z\right)+\sigma_{b}^{2}.
\label{app:variancerecursion}
\end{equation}
When it exists, the large-depth fixed point is
\begin{equation}
q^{*}\equiv\lim_{l\to\infty} q^{\,l}_{aa}.
\end{equation}
For bounded activations (e.g.\ $\tanh$) the fixed point exists for any $\sigma_{w}^{2},\sigma_{b}^{2}$. For unbounded activations (e.g.\ ReLU) it exists only in part of parameter space; outside that region the recursion can run away and the typical preactivation scale diverges with depth.

We now consider the correlations between two distinct inputs $a,b$. Mean-field predicts that different Neurons decorrelate at leading order, so we define
\begin{equation}
\mathbb{E}\!\left[z^{\,l}_{i;a}z^{\,l}_{j;b}\right]
=
q^{\,l}_{ab}\,\delta_{ij}.
\end{equation}
The corresponding mean-field recursion is
\begin{equation}
q^{\,l}_{ab}
=
\sigma_{w}^{2}\int Dz_{1}\,Dz_{2}\,\phi(u_{1})\,\phi(u_{2})
+\sigma_{b}^{2},
\label{eq:mean_field_recurssion}
\end{equation}
with
\begin{align}
u_{1}&=\sqrt{q^{\,l-1}_{aa}}\,z_{1},\qquad 
...
u_{2}&=\sqrt{q^{\,l-1}_{bb}}\left(c^{\,l-1}_{ab} z_{1}+\sqrt{1-\left(c^{\,l-1}_{ab}\right)^{2}}\,z_{2}\right).
\end{align}
After the variance dynamics has relaxed to a common fixed point $q^{*}$ (so $q^{\,l}_{aa}=q^{\,l}_{bb}=q^{*}$), this induces a one-dimensional correlation map $c^{\,l}_{ab}=F(c^{\,l-1}_{ab})$. In the absence of dropout, perfect alignment is always a fixed point: $c^{*}=1$ solves $c^{*}=F(c^{*})$.

To probe its stability, one linearizes the map at $c=1$, defining the DIP susceptibility
\begin{equation}
\chi_{1}
\equiv
\left.\frac{\partial c^{\,l}_{ab}}{\partial c^{\,l-1}_{ab}}\right|_{c_{ab}=1}
...
\sigma_{w}^{2}\int Dz \,\big[\phi'(\sqrt{q^{*}}\,z)\big]^{2}.
\end{equation}
(In the language of \cite{roberts2022principlesdeeplearningtheory}, this is the angular susceptibility $\chi_{\perp}$.) This creates a phase diagram with three regimes:
\begin{itemize}
\item $\chi_{1}<1$: ordered regime. The fixed point $c^{*}=1$ is attractive: different inputs become asymptotically aligned, and gradients typically vanish with depth.
\item $\chi_{1}>1$: chaotic regime. Perfect alignment is unstable, correlations flow to a nontrivial fixed point $c^{*}<1$, and gradients typically explode with depth.
\item $\chi_{1}=1$: critical regime (the edge-of-chaos). The depth scale controlling cross-input correlations can become arbitrarily large.
\end{itemize}


To show the power of criticality, we will derive a well known initialization based purely on demanding criticality: He \cite{he2015delvingdeeprectifierssurpassing}. Let us take $\phi(z)=\max(0,z)$, so that $\phi'(z)=\theta(z)$ and
\begin{equation}
\int Dz\,\big(\phi'(\sqrt{q^{*}}\,z)\big)^{2}
=
\int Dz\,\theta(z)
=
\frac{1}{2}.
...
Hence
\begin{equation}
\chi_{1}=\frac{\sigma_{w}^{2}}{2},
\qquad
\chi_{1}=1 \ \Longrightarrow\  \sigma_{w}^{2}=2,
\end{equation}
which is precisely the He  variance condition (variance $2/N$) for ReLU. At the same time, the ReLU variance recursion simplifies to
\[
q^{\,l}_{aa}=\frac{\sigma_w^2}{2}\,q^{\,l-1}_{aa}+\sigma_b^2,
\]
so for $\sigma_b^2>0$ the fixed point is $q^{*}=\sigma_b^2/(1-\sigma_w^2/2)$ when $\sigma_w^2<2$ and ceases to exist at $\sigma_w^2\ge 2$ (diverging as $\sigma_w^2\to 2^{-}$). Thus, if one insists on $\sigma_b^2>0$, tuning exactly to $\chi_1=1$ is incompatible with a finite $q^{*}$ for ReLU, and in practice one often works slightly subcritical (or sets biases to zero) to keep the signal scale under control. For smooth bounded $\phi$, changing $\sigma_{b}^{2}$ shifts $q^{*}$ and therefore shifts the typical slope $\phi'(\sqrt{q^{*}}z)$, so the critical value of $\sigma_w^2$ becomes $\sigma_b^2$-dependent.

Beyond the phase diagram, these recursions "grow" characteristic depth scales. Iterating random affine maps induces an effective coarse-graining in depth akin to the renormalization group flow in theoretical physics, coined representation group flow in the ML context \cite{roberts2022principlesdeeplearningtheory}: information about norms and angles relaxes exponentially fast toward their fixed points. Concretely, perturb the variance around $q^{*}$ by an infinitesimal amount,
\begin{equation}
q^{\,l}_{aa} = q^{*} + \epsilon^{\,l}_{q}.
\end{equation}
Expanding \eqref{app:variancerecursion} to first order yields
\begin{equation}
\epsilon^{\,l+1}_{q}
=
\epsilon^{\,l}_{q}
...
\chi_{1}
+
\sigma_{w}^{2}\int Dz\,\phi''(\sqrt{q^{*}}z)\,\phi(\sqrt{q^{*}}z)
\Bigg].
\end{equation}
Defining
\begin{equation}
...
\epsilon^{\,l}_{q}
=
\epsilon^{\,0}_{q}\,r_{q}^{\,l}
=
\epsilon^{\,0}_{q}\,e^{-l/\xi_{q}},
\qquad
\xi_{q}^{-1}\equiv -\log|r_{q}|.
\label{app:corrq}
\end{equation}

Similarly, perturbing the correlation around its fixed point $c^{*}$ gives an exponential approach with depth scale
\begin{align}
&\xi_{c}^{-1}
=
-\log\!\left[
...
\right],
\\
&u_{1}^{*}=\sqrt{q^{*}}\,z_{1},\quad
u_{2}^{*}=\sqrt{q^{*}}\!\left(c^{*}z_{1}+\sqrt{1-(c^{*})^{2}}\,z_{2}\right).
\end{align}
Heuristically, $\xi_{c}$ controls how far distinctions between different inputs survive with depth. In the ordered phase $c^{*}=1$, so $u_{1}^{*}=u_{2}^{*}$ and $\xi_{c}^{-1}=-\log\chi_{1}$, hence $\chi_{1}\to 1$ implies $\xi_{c}\to\infty$. Meanwhile, $\xi_{q}$ controls how quickly single-input norms settle to $q^{*}$. For smooth saturating activations such as $\tanh$, the curvature term is typically negative, so even at $\chi_{1}=1$ one often finds $|r_{q}|<1$ and $\xi_{q}$ stays finite and relatively short.

In this sense, the edge-of-chaos is primarily about $\xi_{c}$ rather than $\xi_{q}$, except in special cases. For ReLU, $\phi''=0$ away from the origin (so the curvature contribution vanishes in the Gaussian averages), and one has $r_{q}=\chi_{1}$. In that case $\xi_{q}$ and $\xi_{c}$ essentially coincide and both diverge as $\chi_{1}\to 1$, provided a finite $q^{*}$ exists. This is one concrete reason why ReLU-style near-critical initializations are comparatively forgiving in practice.

MFT provides a good approximation in very wide networks, especially through the first few layers. At large but finite width, deviations from Gaussian mean-field behavior can be organized perturbatively in the depth-to-width aspect ratio \(L/N\). Accordingly, in our experiments we work in a controlled regime with \(L/N \ll 1\) (i.e., \(N \gg L\)). Near criticality, the correlation length becomes large, which slows relaxation to the fixed point and typically amplifies sensitivity to finite-width and other non-Gaussian corrections. Hence, when probing critical power laws while maintaining dropout, we keep the dropout rate parametrically small but non-negligible: taking dropout to be infinitesimal would require prohibitively deep networks to obtain precise estimates of the correlation length and related quantities, making the experiments computationally expensive.
...
\[
\mathbb E[u_1^2]=\mathbb E[u_2^2]=\bar q^*,
\qquad
\mathbb E[u_1u_2]=c\,\bar q^*,
\qquad
c\in[-1,1].
\]
...
\bar F_\rho(c)
\;\equiv\;
\frac{\bar q^{l+1}_{ab}}{\bar q^*}
\;=\;
\frac{\sigma_w^2\,\mathbb E\!\big[\phi(u_1)\phi(u_2)\big]+\sigma_b^2}{\bar q^*}.
...
\bar F_\rho'(c)
\;=\;
\frac{\sigma_w^2}{\bar q^*}\,
\frac{\partial}{\partial c}\,
\mathbb E\!\big[\phi(u_1)\phi(u_2)\big].
...
\begin{equation}
\frac{\mathrm d}{\mathrm dc}G(c)
\;=\;
\mathbb E\!\left[
\frac{\partial^2}{\partial v_1\,\partial v_2}
\Big(
\phi\!\big(\sqrt{\bar q^*}\,v_1\big)\,
\phi\!\big(\sqrt{\bar q^*}\,v_2\big)
\Big)
\right].
\label{eq:price_step}
\end{equation}
...
G'(1)
=
\bar q^*\int Dz\,\big[\phi'(\sqrt{\bar q^*}\,z)\big]^2.
\]
...
\mathbb E\!\Big[
\phi'(\sqrt{\bar q^*}\,v_1)\phi'(\sqrt{\bar q^*}\,v_2)
\Big]
=
\bar q^*\,
\mathbb E\!\Big[
...
\[
\left.\frac{\partial^2}{\partial c^2}\,
\mathbb E\!\big[\phi(u_1)\phi(u_2)\big]\right|_{c=1}
=
(\bar q^*)^2\int Dz\,\big[\phi''(\sqrt{\bar q^*}\,z)\big]^2.
\]
...
\begin{equation}
  f(t,m;h)
  =
  -\frac{t}{2}\,m^2 + \frac{g_\rho}{6}\,m^3 - h\,m,
...
\[
f_{\rm on}(t,0)=0 \quad (t\le 0),
\qquad
f_{\rm on}(t,0)=-\frac{2}{3g_\rho^{2}}\,t^{3}\quad (t\ge 0).
\]
...
The mean-field correlation map for ReLU admits a closed form. Writing \(c\in[-1,1]\) for the preactivation correlation at some depth,
\begin{align}
F_{\mathrm{ReLU}}(c)
&=
\frac{1}{\pi}
...
\begin{equation}
  h
  \;=\;
  \kappa\,m^{3/2} - t\,m + \mathcal O(m^2),
  \qquad
  t\equiv \chi_\rho-1.
  \label{eq:eos_kink_correct}
\end{equation}
Along the critical isotherm \(t=0\), this gives
\begin{equation}
m^*
\;\sim\;
\left(\frac{h}{\kappa}\right)^{2/3},
\qquad (t=0),
\label{eq:kink_mstar}
\end{equation}
so the smooth exponent \(\delta=2\) is replaced by
\begin{equation}
  m \propto h^{1/\delta_{\mathrm{kink}}},
  \qquad
  \delta_{\mathrm{kink}}=\frac{3}{2}.
\label{eq:kink_delta}
\end{equation}
...
\begin{equation}
  f_{\mathrm{kink}}(t,m;h)
  \;=\;
  -\frac{t}{2}\,m^2
  + \frac{2\kappa}{5}\,m^{5/2}
  - h\,m,
...
\[
f_{\mathrm{kink,on}}(t,0)=0 \quad (t\le 0),
\qquad
f_{\mathrm{kink,on}}(t,0)=-\frac{1}{10\kappa^{4}}\,t^{5}\quad (t\ge 0).
\]
...
\end{equation}
which reproduces the characteristic \(1-c_\ell\sim \ell^{-2}\) relaxation of ReLU activations at the edge-of-chaos.
...
1-m+\kappa\,m^{3/2}+{\cal O}(m^{2}),
\qquad
\kappa=\frac{2\sqrt{2}}{3\pi},
\label{eq:app_relu_expand}
\end{align}
starting from the closed form ReLU correlation map
\begin{align}
F_{\mathrm{ReLU}}(c)
&=
\frac{1}{\pi}
...
\sqrt{2m-m^{2}}
=
\sqrt{2m}\,\sqrt{1-\frac{m}{2}}
\nonumber\\
&=
\sqrt{2m}
\left(1-\frac{m}{4}+{\cal O}(m^{2})\right)
=
\sqrt{2}\,m^{1/2}-\frac{\sqrt{2}}{4}m^{3/2}+{\cal O}(m^{5/2}).
...
\cos\theta
=
1-\frac{\theta^{2}}{2}+\frac{\theta^{4}}{24}+{\cal O}(\theta^{6}),
...
\cos^{-1}(1-m)
=
\sqrt{2}\,m^{1/2}+\frac{\sqrt{2}}{12}m^{3/2}+{\cal O}(m^{5/2}).
...
\begin{align}
\sqrt{1-c^{2}}+\bigl(\pi-\cos^{-1}c\bigr)c
&=
\pi(1-m)
+
\left(
-\frac{\sqrt{2}}{4}+\frac{11\sqrt{2}}{12}
\right)m^{3/2}
+
{\cal O}(m^{2})
\nonumber\\
...
\begin{equation}
\mathbb{E}[u_{1}]=\mathbb{E}[u_{2}]=0,
\qquad
\mathbb{E}[u_{1}^{2}]=\mathbb{E}[u_{2}^{2}]=1,
\qquad
\mathbb{E}[u_{1}u_{2}]=c.
\end{equation}
...
\begin{equation}
s=\frac{u_{1}+u_{2}}{\sqrt{2}},
\qquad
d=\frac{u_{1}-u_{2}}{\sqrt{2}}.
\end{equation}
A direct covariance computation gives
\begin{equation}
\mathrm{Var}(s)=1+c=2-m,
\qquad
\mathrm{Var}(d)=1-c=m,
\qquad
\mathbb{E}[sd]=0.
...
\begin{equation}
\phi(u)=a_-\,u+\big(a_+-a_-\big)\,\mathrm{ReLU}(u)+r(u),
\qquad
r(u)=o(u)\ \ \text{as }u\to 0,
\end{equation}
...
\[
Dz \equiv \frac{e^{-z^2/2}}{\sqrt{2\pi}}\,dz,
\qquad
\langle f,g\rangle \equiv \int Dz\, f(z)\,g(z).
\]
...
\]
Assuming $\phi$ is weakly differentiable with $f \in H^1(Dz)$, Price's theorem yields the correlation susceptibility \cite{schoenholz2017deepinformationpropagation}
\begin{equation}
...
\chi
=
\left(1-\frac{\sigma_b^2}{q^*}\right)
\frac{\int Dz\, (f')^2}{\int Dz\, f^2}
\equiv
\left(1-\frac{\sigma_b^2}{q^*}\right)\,\mathcal{Q}[f],
\qquad
\mathcal{Q}[f]\equiv \frac{\|f'\|_2^2}{\|f\|_2^2}.
\end{equation}
...
A natural orthonormal eigenbasis of \eqref{eq:OU-eigenvalue} is given by the normalized Hermite polynomials
\[
h_n(z) \equiv \frac{1}{\sqrt{2^n n!}}\,H_n\!\left(\frac{z}{\sqrt{2}}\right),
\qquad
\langle h_n,h_m\rangle = \delta_{nm},
\]
...
\[
h_n'(z) = \sqrt{n}\,h_{n-1}(z),
\qquad
\left(-\partial_z^2 + z\,\partial_z\right)h_n(z)=n\,h_n(z),
\qquad
\mathcal{Q}[h_n]=n.
\]
...
\[
f(z)=\sum_{n\ge 0} a_n\,h_n(z),
\qquad
a_n=\langle f,h_n\rangle,
\qquad
\sum_{n\ge 0} a_n^2 = \int Dz\, f^2,
\]
we obtain
\[
...
\[
F(c)=\frac{\sigma_w^2\sum_{n\ge 0} a_n^2 c^n + \sigma_b^2}{q^*},
\qquad
q^*=\sigma_w^2\sum_{n\ge 0} a_n^2+\sigma_b^2.
\]
...
\left(
\sum_{n\ge 1} n\,a_n^2
+ \sum_{n\ge 0}\sqrt{(n+1)(n+2)}\,a_n a_{n+2}
\right).
\end{equation}
...
\[
\chi=\sigma_w^2\int Dz\,\theta(u)=\frac{\sigma_w^2}{2},
\qquad
\chi=1 \Rightarrow \sigma_w^2=2.
\]
In our scale-fixed convention \eqref{eq:chi-factorization}, ReLU has $\mathcal{Q}[f]=1$ (for $f(z)\propto \max(0,z)$), and hence $\chi=(1-\sigma_b^2/q^*)$; achieving $\chi=1$ in that convention forces $\sigma_b^2=0$. In Hermite space, the kink generates a slow (power-law) tail in $|a_n|$, and the resulting series for $F(c)$ develops a branch point as $c\to 1$; the non-analyticity originates from the collective contribution of high-degree modes.
...
\hline
\(n\) & \(h_{n}(z)\) parity & \(a_{n}\) for \(\mathrm{ReLU}(z)\) & \(a_{n}\) for \(\tanh(z)\) \\
\hline
...
\hline
\end{tabular}
\caption{Orthonormal Hermite coefficients \(a_{n}=\int Dz\,f(z)h_{n}(z)\) for two standard activations, shown for the unscaled choices $f(z)=\mathrm{ReLU}(z)$ and $f(z)=\tanh(z)$. For $f(z)=\tanh(s z)$, use \(a_{n}(s)=\int Dz\,\tanh(s z)h_{n}(z)\), with \(a_{2k}(s)=0\) by odd parity.}
\end{table}

\begin{figure}[h]
    \centering
    \includegraphics[width=0.7\linewidth]{figures/theory/hermite_decomposition.png}
...
\begin{equation}
f(z)=\sum_{n\ge 0} a_{n}\,h_{n}(z),
\qquad
a_{n}=\int Dz\,f(z)\,h_{n}(z),
\qquad
\int Dz\,h_{n}(z)h_{m}(z)=\delta_{nm}.
\end{equation}
With physicists Hermites \(H_{n}\),
\begin{equation}
H_{n}(x)\equiv (-1)^{n}e^{x^{2}}\frac{d^{n}}{dx^{n}}e^{-x^{2}},
\qquad
h_{n}(z)\equiv \frac{1}{\sqrt{2^{n}n!}}\,H_{n}\!\left(\frac{z}{\sqrt{2}}\right).
\end{equation}

\subsubsection{ReLU}

Let \(f(z)=\mathrm{ReLU}(z)=\max(0,z)\). The coefficients have a closed form.
\begin{equation}
a_{0}=\frac{1}{\sqrt{2\pi}},
\qquad
a_{1}=\frac{1}{2},
\qquad
a_{2k+1}=0 \ \ \text{for } k\ge 1,
\end{equation}
and for \(k\ge 1\),
\begin{equation}
a_{2k}
=
\frac{(-1)^{k-1}(2k-3)!!}{\sqrt{2\pi(2k)!}}.
\end{equation}
We use the convention \((-1)!!=1\). If instead \(f(z)=\mathrm{ReLU}(\sqrt{q^{*}}\,z)\), then all coefficients scale as \(a_{n}\mapsto \sqrt{q^{*}}\,a_{n}\).
...
This appendix collects the experimental details and additional figures supporting the scheduling predictions. Code for the experiments and figure generation is available at \url{https://github.com/luklacasito/dropout-mft-experiments}.
\subsection{Mean-Field Fits and Scaling Collapse}
Table~\ref{tab:mf_vs_fit_exponents} reports exponents extracted from iterated MFT recursions \eqref{variancerecursion}.
\begin{table}[ht]
\centering
\begin{tabular}{lcc}
\toprule
...
$\beta_{\mathrm{ReLU}}$ & $1.9642 \pm 0.0019$ & $2$ \\
$p_{\mathrm{tanh}}$ & $1.0048 \pm 0.0000$ & $1$ \\
$p_{\mathrm{ReLU}}$ & $1.9870 \pm 0.0001$ & $2$ \\
$1/\delta_{\mathrm{tanh}}$ & $0.5171 \pm 0.0007$ & $1/2$ \\
$1/\delta_{\mathrm{ReLU}}$ & $0.6656 \pm 0.0001$ & $2/3$ \\
$\nu_{\rho,\mathrm{tanh}}$ & $0.4716 \pm 0.0014$ & $1/2$ \\
$\nu_{\rho,\mathrm{ReLU}}$ & $0.3528 \pm 0.0010$ & $1/3$ \\
\bottomrule
\end{tabular}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/theory/kinked_scaling_collapse.png}
...
\begin{figure*}[ht]
    \centering
    \includegraphics[width=0.8\textwidth]{figures/experiments/mlp/dropout_schedules_overfit.png}
    \caption{Finite-width MLP training and test curves at fixed mean dropout budget $\overline{h}$ (with $0 \le h_\ell \le h_{\max}$). We compare uniform dropout, linear ramps (increasing/decreasing with depth), and step schedules that concentrate dropout in either
...
\begin{tabular}{lccc}
\hline\hline
Schedule & $\xi_{\mathrm{eff}}$ & $L/\xi_{\mathrm{eff}}$ & Best Test Accuracy (\%) \\
\hline
Constant            & $3.20$   & $1.87$ & $42.54 \pm 0.10$ \\
Step (late)         & $5.09$   & $1.18$ & $40.56 \pm 0.11$ \\
Step (early)        & $5.09$   & $1.18$ & $43.13 \pm 0.07$ \\
Linear (increasing) & $3.73$   & $1.61$ & $41.01 \pm 0.07$ \\
Linear (decreasing) & $3.73$   & $1.61$ & $43.11 \pm 0.09$ \\
No dropout          & $\infty$ & $0$    & $38.77 \pm 0.10$ \\
\hline\hline
\end{tabular}
...
To disentangle allocation from total dropout strength, we compare constant schedules with fields $\bar h,2\bar h,3\bar h$ against step schedules using $2\bar h$ in the first half or $3\bar h$ in the first third of the network (Fig.~\ref{fig:triple_dropout}, Tab.~\ref{tab:triple_summary}).
 \begin{figure}[h]
     \centering
     \includegraphics[width=1.0\linewidth]{figures/experiments/mlp/dropout_budget_comparison.png}
...
Schedule & $\xi_{\mathrm{eff}}$ & $L/\xi_{\mathrm{eff}}$ & Best Test Accuracy (\%) \\
\hline
No dropout          & $\infty$ & $0.00$ & $38.77 \pm 0.10$ \\
Constant            & $3.20$   & $1.87$ & $42.54 \pm 0.10$ \\
Step (early)        & $5.09$   & $1.18$ & $43.13 \pm 0.07$ \\
Big step (1/3)      & $6.67$   & $0.90$ & $43.29 \pm 0.08$ \\
Double ($2\bar{h}$) & $2.54$   & $2.36$ & $41.46 \pm 0.08$ \\
Triple ($3\bar{h}$) & $2.22$   & $2.70$ & $33.94 \pm 0.22$ \\
\hline\hline
\end{tabular}
...
To test whether scheduling is tied to one budget, width, or activation, we ran CIFAR-10 MLP sweeps (Figs.~\ref{fig:mlp_sweeps} and \ref{fig:gelu_h_sweep}). The ReLU sweeps probe mean-field boundaries: at \(\bar h=0.15\), Step/Big step use nonperturbative local fields \(0.30/0.45\); at \(N=64\), finite-width non-Gaussianity weakens the correlation-length prediction. The GELU MLP sweep gives the same \(\bar h=0.1\) ordering--constant \(41.96\pm0.16\), Step \(42.24\pm0.15\), Big step \(42.46\pm0.12\)--and Big step falls to \(38.97\pm0.23\) at \(\bar h=0.15\), matching this perturbative-boundary picture.

\begin{figure*}[t]
    \centering
    \subfigure[Sweep over mean dropout field at width \(N=256\).]{
        \includegraphics[width=0.45\textwidth]{figures/experiments/sweeps/h_sweep_schedule_comparison.png}
    }
    \subfigure[Schedule advantage over constant dropout at \(\bar h=0.1\).]{
        \includegraphics[width=0.45\textwidth]{figures/experiments/sweeps/width_sweep_baseline_advantage.png}
    }
...
\begin{figure*}[h]
    \centering
    \includegraphics[width=0.95\textwidth]{figures/experiments/transformer/dropout_schedules_cropped.png}
    \caption{Finite-width Vision Transformer training and test curves at fixed mean dropout budget $\overline{h}$. We compare the no-dropout baseline, uniform dropout, decreasing linear ramps, and early step schedules. The cropped view shows epochs 20--75 for readability; the full curve is also in App.~\ref{app:experimental_info}.}
    \label{fig:dropout_regimes_transformer}
\end{figure*}
\begin{figure}[t]
    \centering
    \includegraphics[width=0.95\linewidth]{figures/experiments/transformer/dropout_schedules_full.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figures/experiments/transformer/component_ablations.png}
...
\begin{table}[h]
    ...
    \begin{tabular}{l l}
        ...
        Mean Dropout Field ($\bar{h}$) & 0.1 \\
        Max Dropout Field ($h_{\max}$) & 0.2 \\
        Schedules & None, Constant, Reverse Step \\
...
\begin{table}[H]
    ...
    \begin{tabular}{l c c}
        ...
        Both (Step early) & $75.47 \pm 0.27$ & $75.39 \pm 0.29$ \\
        \bottomrule
    \end{tabular}
...
\label{subsec:loss_improvements}

Table~\ref{tab:loss_improvements} summarizes the corresponding test-loss gains. All entries are computed relative to the constant-dropout baseline in the same experiment, using the saved per-seed test-loss curves. We report both the final-epoch loss reduction and the reduction in the best test loss attained during training; positive numbers indicate lower loss than constant dropout.

\begin{table*}[t]
\centering
\scriptsize
\begin{tabular}{@{}L{0.18\textwidth}L{0.33\textwidth}L{0.18\textwidth}L{0.18\textwidth}@{}}
\toprule
\textbf{Experiment} & \textbf{Dataset and architecture} & \textbf{Final test loss reduction} & \textbf{Best test loss reduction} \\
\midrule
MLP schedule sweep, Fig.~\ref{fig:dropout_regimes_overfit}
&
CIFAR-10 subset with 5,000 training images; near-critical ReLU MLP with width 256 and depth \(2\xi\).
&
Step (early): \(17.9\%\)
&
Step (early): \(1.4\%\)
...
&
Same CIFAR-10 near-critical ReLU MLP setup; compares spatial allocation against larger uniform dropout budgets.
&
Double: \(24.3\%\)
&
Big step: \(2.1\%\)
\\
\addlinespace[1pt]
ViT schedule sweep, Fig.~\ref{fig:dropout_regimes_transformer}
&
Full CIFAR-100; 12-layer ViT with embedding dimension 128, 16 heads, MLP ratio 4, and patch size 4.
&
Linear (decreasing): \(4.2\%\)
&
Step (early): \(2.2\%\)
\\
\addlinespace[1pt]
ViT component ablations, Fig.~\ref{fig:ablations}
&
Full CIFAR-10; same 12-layer ViT, with dropout applied to attention only, MLP only, or both blocks.
&
Both blocks, step (early): \(6.3\%\)
&
Both blocks, step (early): \(0.9\%\)
\\
\addlinespace[1pt]
GELU MLP smooth check
&
CIFAR-10 subset with 5,000 training images; smooth-activation MLP with width 256, depth 6, and \(\bar h=0.1\).
&
Big step: \(29.8\%\)
&
Big step: \(2.5\%\)
...
\bottomrule
\end{tabular}
\caption{Summary of test-loss improvements over the corresponding constant-dropout baseline. ``Final'' uses the last recorded epoch, while ``best'' uses the minimum test loss attained by each run before averaging.}
