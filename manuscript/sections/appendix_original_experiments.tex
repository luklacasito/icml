...
\begin{figure*}[ht]
 ...
 \caption{Kinked counterpart to Fig.~\ref{fig:uni_scaling}. Each curve holds the field $h$ fixed by setting $\rho=\chi/(\chi+h)$ in the formal \relu{} channel above. The points solve the full map; rescaling uses its local kink coefficient $\kappa_{\rm loc}=\chi\,2\sqrt{2}/(3\pi)$. The horizontal coordinate reverses the sign of $u$ in Eq.~\eqref{eq:def_u}, evaluated with this coefficient, which approaches the critical value as $\chi\to1$. The black curve is the leading equation of state, and the remaining departures at larger fields come from higher-order terms. This formal comparison does not require independently tunable finite-variance \relu{} initializations.}
