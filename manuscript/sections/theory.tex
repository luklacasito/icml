...
Since \(\xi_{\rm eff}\) in \eqref{eff_corr} is symmetric in \(\{h_\ell\}\), any permutation of a saturated schedule pays the same cost within this approximation. We propose breaking the tie by maximizing downstream exposure to the perturbation (App.~\ref{app:regularization_reach}). Apply dropout only at layer \(\ell\). For masked and clean activations \(\tilde y_k^{(\ell)}\) and \(y_k\) at a later layer \(k\), define their angular decorrelation as
\begin{equation}
\eta_k^{(\ell)}
\equiv
