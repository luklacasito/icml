...
\caption{Imagine you have a flowing river and a set amount of ink, with the goal of causing as much corruption as possible at every point: where would you drop it? Framing current dropout practice through this lens makes it clear why front-loading is more effective: the current carries the ink forward.}
...
To make the toy model precise, use normalized depth $x=\ell/L$ and write $j(x)=h(x)/\bar h$ for the injection density, with $\bar h>0$ and $\int_0^1j(x)\,dx=1$. For the river in Fig.~\ref{fig:river_reach}, where the ink does not fade as it travels downstream, the accumulated exposure at $x$ and its average along the river are
\begin{align}
E(x)&=\int_0^x j(s)\,ds,\\
