...
This distinction originally emerged from the following variational question: among activations of fixed scale, which functions make the correlation length of the near-critical Gaussian channel as large as possible? As a tractable part of that question, we extremize the angular susceptibility at fixed variance scale; the resulting eigenfunctions are Hermite polynomials. When standard nonlinearities are decomposed in that basis, two qualitatively different spectra emerge. Smooth activations such as $\tanh$ have rapidly decaying Hermite coefficients, so much of their spectral weight is concentrated in the first few modes. Kinked activations such as \relu{} have power-law tails instead: the kink keeps exciting higher Hermite modes, leaving a broader spectrum in the correlation map. This spectral split was the original diagnostic for the smooth/kinked universality classes, which later reappears in the dropout scaling laws. See App.~\ref{sec:var-opt-act} for detailed calculations.
...
For \relu{}, imagine two nearly identical signals arriving on opposite sides of zero: one passes through, while the other is clipped. At a fixed nonzero Gaussian variance and correlation $c$ close to one, the signals differ by an amount of order $\sqrt{1-c}$, so a fraction of this order can straddle the kink. Within this small region, their squared activation difference is of order $1-c$, giving a contribution of order $(1-c)^{3/2}$. At the fixed point, where $m=1-c^*$, this produces the $m^{3/2}$ correction, while the smooth class above has an $m^2$ term (App.~\ref{app:tube_argument_kink}).
...
\caption{\textbf{Ink as a toy model of regularization reach.} Each row introduces the same total amount of ink, spread uniformly or at twice the rate over one half of the river. Shading represents cumulative upstream injection without attenuation. The percentages are mean exposure in this toy model; they do not represent measured improvements in loss or accuracy. Diagrams adapted from the author's presentation.}
