...
def test_historical_rng_fixture_and_numeric_type_sensitivity():
    ...
    assert seed_streams(arm["spec"]) == {
        ...
        "base_seed": 100,
        "initialization_seed": 2034274571,
        "minibatch_seed": 3044408285,
        "dropout_seed": 98623406,
        "dropout_crn_group": "b173e876aefef39b75ed",
    }
    edited = deepcopy(arm)
...
def test_model_forward_for_each_retained_input_family():
    ...
    for dataset, shape in (
        ("fi2010", (2, 100, 40)),
        ("openml_jannis", (2, 54, 1)),
        ("tiny_imagenet", (2, 3, 64, 64)),
        ("speech_commands", (2, 1, 64, 64)),
    ):
...
def test_checkpoint_smoke_reloads_distinct_best_and_final(tmp_path):
    ...
    assert (
        result["test"]["best_validation_checkpoint"]["loss"]
        < result["test"]["fixed_final_epoch"]["loss"]
    )
...
def test_test_labels_cannot_change_training_or_checkpoint_selection(tmp_path):
    ...
    modified = replace(
        bundle,
        test=TensorDataset(bundle.test.tensors[0], torch.zeros(12, dtype=torch.long)),
    )
...
def test_splits_are_disjoint_balanced_and_temporally_embargoed():
    ...
    parts = data._balanced_split(
        labels, train_size=24, validation_size=8, test_size=12, seed=20260812
    )
    ...
    train, validation, test = data._anchored_split(
        70_200, train_size=40_000, validation_size=10_000, test_size=20_000, embargo=100
    )
...
def test_standardization_is_train_only_and_tiny_modes_remain_distinct():
    original = np.asarray([[1.0, 3.0], [3.0, 7.0], [100.0, 200.0]], dtype=np.float32)
    changed = original.copy()
    changed[-1] *= 100
    for normalize in (
        data._standardize,
        data._standardize_inplace_v15,
        data._standardize_inplace,
    ):
...
def test_cache_payload_and_streamed_rows_reject_corruption(tmp_path):
    ...
    np.testing.assert_array_equal(
        data._load_compressed_npz_rows(path, "features_mlp", selected, chunk_bytes=31),
        mlp[selected],
    )
    mlp = mlp.copy()
    mlp[0, 0] = -1
